...
if __name__ == "__main__":
    ...
    if args.dataset == "MNIST":
        ...
        test_iter = DataLoader(datasets.MNIST(root=args.path, train=False, download=True, transform=transforms.ToTensor()), batch_size=batch_size, shuffle=True, num_workers=7, pin_memory=True, drop_last=True)
        data_dim = 28 * 28
    else:
        ...
        test_iter = DataLoader(datasets.CIFAR10(root=args.path, train=False, download=True, transform=transforms.ToTensor()), batch_size=batch_size, shuffle=True, num_workers=7, pin_memory=True, drop_last=True)
        data_dim = 32 * 32 * 3
    # model
